...
# Import our existing service connections
from custom_functions import agent_services, PINECONE_AVAILABLE, NEO4J_AVAILABLE
...
def read_s3_data_crud(bucket: str, key: str) -> Dict[str, Any]:
    ...
    """
    try:
        response = agent_services.s3_client.get_object(Bucket=bucket, Key=key)
...
def search_pinecone_crud(query_vector: List[float], limit: int = 10) -> Dict[str, Any]:
    ...
    try:
        ...
        results = agent_services._pinecone_index.query(
            vector=query_vector,
            top_k=limit,
...
def read_dynamodb_crud(table_name: str, key: Dict[str, Any]) -> Dict[str, Any]:
    ...
    """
    try:
        table = agent_services.dynamodb.Table(table_name)
...
def generate_embedding_crud(text: str) -> Dict[str, Any]:
    ...
    """
    try:
        if not agent_services._embedding_model:
            return {
                'success': False,
                'error': 'Embedding model not available'
            }
        
        embedding = agent_services._embedding_model.encode(text)
...
def upsert_pinecone_crud(vectors: List[Dict[str, Any]], namespace: str = None) -> Dict[str, Any]:
    ...
    try:
        ...
        response = agent_services._pinecone_index.upsert(
            vectors=vectors,
            namespace=namespace
        )
...
def delete_pinecone_crud(ids: List[str], namespace: str = None) -> Dict[str, Any]:
    ...
    try:
        ...
        response = agent_services._pinecone_index.delete(
            ids=ids,
            namespace=namespace
        )
